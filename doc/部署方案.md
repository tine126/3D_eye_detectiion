...
本项目基于ROS2实现双路IR相机的人脸检测、关键点定位和三维眼睛坐标计算功能。
...
│                                                                                                 │
│  ┌─────────────────────┐                      ┌─────────────────────┐                          │
│  │ eye_position_publisher│                      │   eye_position_3d   │                          │
│  │ 眼睛2D坐标提取节点   │─────────────────────>│ 三维坐标计算节点    │────> /eye_positions_3d   │
│  │                     │                      │                     │                          │
│  │ 左眼/右眼中心坐标   │                      │ 双目三角测量        │                          │
│  │ 双路并行处理        │                      │ 3D坐标输出 (mm)     │                          │
│  └─────────────────────┘                      └─────────────────────┘                          │
...
| 眼睛2D坐标 | /eye_positions_left | /eye_positions_right |
| 眼睛3D坐标 | /eye_positions_3d | - |
...
colcon build --packages-select \
    ...
    eye_position_publisher \
    eye_position_3d

# 编译统一启动包
colcon build --packages-select tcl_eye_tracking_bringup
...
# 眼睛2D坐标提取节点
colcon build --packages-select eye_position_publisher

# 三维坐标计算节点
colcon build --packages-select eye_position_3d
...
```bash
# 终端5: 启动eye_position_publisher
source /opt/tros/humble/setup.bash
source ~/ros2_ws/install/setup.bash

ros2 launch eye_position_publisher eye_position_publisher.launch.py \
    log_level:=warn
```
...
```bash
# 终端6: 启动eye_position_3d
source /opt/tros/humble/setup.bash
source ~/ros2_ws/install/setup.bash

ros2 launch eye_position_3d eye_position_3d.launch.py \
    use_manual_camera_params:=true \
    manual_fx:=455.0 \
    manual_fy:=455.0 \
    manual_cx:=640.0 \
    manual_cy:=400.0 \
...
ros2 launch eye_position_3d eye_position_3d.launch.py \
    use_manual_camera_params:=true \
    manual_fx:=455.0 manual_fy:=455.0 \
    manual_cx:=640.0 manual_cy:=400.0 \
    manual_baseline_mm:=95.0 &
...
### 6.4 eye_position_publisher 参数

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `left_sub_topic` | `/face_landmarks_detection_left` | 左IR关键点输入 |
| `left_pub_topic` | `/eye_positions_left` | 左IR眼睛坐标输出 |
| `right_sub_topic` | `/face_landmarks_detection_right` | 右IR关键点输入 |
| `right_pub_topic` | `/eye_positions_right` | 右IR眼睛坐标输出 |

### 6.5 eye_position_3d 参数

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `left_eye_topic` | `/eye_positions_left` | 左IR眼睛2D坐标输入 |
| `right_eye_topic` | `/eye_positions_right` | 右IR眼睛2D坐标输入 |
| `output_topic` | `/eye_positions_3d` | 3D坐标输出 |
| `min_disparity` | `5.0` | 最小视差(像素) |
| `max_disparity` | `200.0` | 最大视差(像素) |
| `min_depth_mm` | `200.0` | 最小深度(mm) |
| `max_depth_mm` | `2000.0` | 最大深度(mm) |
| `max_y_diff` | `10.0` | 最大Y坐标差异(像素) |
| `use_manual_camera_params` | `false` | 是否使用手动配置 |
| `manual_fx` | `0.0` | 手动焦距x |
| `manual_fy` | `0.0` | 手动焦距y |
| `manual_cx` | `0.0` | 手动主点x |
| `manual_cy` | `0.0` | 手动主点y |
| `manual_baseline_mm` | `0.0` | 手动基线距离(mm) |
...
# 检查眼睛2D坐标输出频率
ros2 topic hz /eye_positions_left
...
# 查看眼睛2D坐标
ros2 topic echo /eye_positions_left
...
**eye_position_3d**:
```
Stats: 30.0 fps, 150 msgs, 150 faces, 145 valid (96.7%)
```
